...
path_base <- "C:/"
path_salida <- file.path(path_base, "Mapas_Flotas_NAE")
...
reg_fleets <- read.csv2("NAE_regional_fleets.csv")
acr_fleets <- read.csv2("NAE_acrossfleets.csv")

# Mapas base (usando tus rutas del script de Gloria)
Ecoregions <- sf::st_read("C:/ECOREGIONS_SP_DP001.shp")
...
mapear_flota <- function(nombre_flota) {
  ...
  # Guardar
  ggsave(file.path(path_salida, paste0("Map_NAE_", nombre_flota, ".png")), 
